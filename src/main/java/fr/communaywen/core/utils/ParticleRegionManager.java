...
public class ParticleRegionManager {
    ...
    public static void spawnColoredParticlesInRegion(String regionId, World world, Particle particle, Integer amountPer2Tick, Color color, Integer maxHeight) {
        RegionManager regionManager = WorldGuard.getInstance().getPlatform().getRegionContainer().get(BukkitAdapter.adapt(world));
        if (regionManager == null) return;
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                for (int i = 0; i < amountPer2Tick; i++) {
                    ...
                    world.spawnParticle(particle, particleLocation, 0, color);
                }
            }
        }.runTaskTimer(plugin, 0L, 2L);
    }
